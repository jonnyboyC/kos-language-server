...
type NodeConstructor = Constructor<Expr> | Constructor<Inst> | Constructor;
...
export class Parser {
  ...
  private error = (
    ...
    if (failed.prototype instanceof Expr) {
      return new ParseError(token, failedExpr(failed as {new(): Expr}), message, extraInfo);
    }
